...
export function getBlockShareConfig() {
  ...
  return nativeMessage({
    ...
    response => {
      ...
      return validatedBlockShareConfig(
        response.data,
        BLOCK_SHARE_INVALID_CONFIG_ERROR + ': '
      )
    },
...
function getGithubAuthFromBlockShareConfig(config) {
  ...
  return {
    ...(config.blockStore.authUsername && {
      username: config.blockStore.authUsername
    }),
...
export function validatedBlockShareConfig(configFileObject, errorPrefix) {
  ...
  if (!('blockStore' in configFileObject)) {
    return Promise.reject(
      new Error(errorPrefix + ': Object blockStore is not present')
    )
  }
  ...
  if (
    'authUsername' in blockStoreConfig &&
    typeof blockStoreConfig.authUsername !== 'string'
  ) {
    return Promise.reject(
      new Error(errorPrefix + ': blockStore.username is not a string')
    )
  }
  if (
    'authToken' in blockStoreConfig &&
    typeof blockStoreConfig.authToken !== 'string'
  ) {
    return Promise.reject(
      new Error(errorPrefix + ': blockStore.authToken is not a string')
    )
  }
  if (
    !('repoUsername' in blockStoreConfig) ||
    typeof blockStoreConfig.repoUsername !== 'string'
  ) {
    return Promise.reject(
      new Error(
        errorPrefix +
          ': blockStore.repoUsername must be present and be a string'
      )
    )
  }
  if (
    !('repo' in blockStoreConfig) ||
    typeof blockStoreConfig.repo !== 'string'
  ) {
    return Promise.reject(
      new Error(
        errorPrefix + ': blockStore.repo must be present and be a string'
      )
    )
  }
  if (
    !('github_api_url' in blockStoreConfig) ||
    typeof blockStoreConfig.github_api_url !== 'string'
  ) {
    return Promise.reject(
      new Error(
        errorPrefix +
          ': blockStore.github_api_url must be present and be a string'
      )
    )
  }
