  ...
  return (dispatch, getState) => {
    return projectModel
      ...
      .then(updatedProject => {
        ...
        dispatch(
          logMessage({ type: 'Update Project', project: updatedProject })
        )
      })
  }
}
...
export function editProject(id) {
  return (dispatch, getState) => {
    return projectModel.get(id).then(project =>
      dispatch({
        type: T.EDIT_PROJECT,
        project
      })
...
export function writeProjectConfig(
  ...
  return (dispatch, getState) => {
    ...
    return nativeMessage({
      ...
      .then(() =>
        dispatch(logMessage({ type: 'Write Project Config', project }))
      )
...
export function reloadProjectFiles(project) {
  return (dispatch, getState) => {
    const filepathsToData = files =>
      nativeMessage({
        type: 'readFiles',
        filepaths: files
          .filter(f => f.name.includes('.json'))
          .map(f => f.fullpath)
      }).then(response =>
