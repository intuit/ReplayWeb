...
const OneField = (props) => {
  ...
  return <Form>
    {params && params.map(param => {
      ...
      switch (param.type) {
        case 'object':
          return <Form.Item key={param.name} label={param.name} labelCol={{ span: 4 }} wrapperCol={{ span: 14 }} style={{ marginBottom: 0 }}>
            This parameter needs to be configured manually in the JSON.
          </Form.Item>
        case 'checkbox':
          return <Form.Item key={param.name} label={param.name} labelCol={{ span: 4 }} wrapperCol={{ span: 10 }} style={{ marginBottom: 0 }} extra={param.extra}>
            <Checkbox
              checked={value}
        ...
        case 'select':
          return <Form.Item key={param.name}
            label={param.name}
            labelCol={{ span: 4 }}
            wrapperCol={{ span: 14 }}
          ...
          >
            <Select
              disabled={!props.isCmdEditable && (!props[param.data] || props[param.data].length === 0)}
              showSearch
              placeholder={param.name}
        ...
        default:
          return <Form.Item key={param.name} label={param.name} labelCol={{ span: 4 }} wrapperCol={{ span: 20 }} style={{ marginBottom: 0 }} extra={param.extra}>
            <Input
              value={value}
              disabled={!props.isCmdEditable}
