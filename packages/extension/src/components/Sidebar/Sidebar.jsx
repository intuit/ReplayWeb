...
class Sidebar extends React.Component {
  ...
  changeTestCase = (id) => {
    return new Promise((resolve, reject) => {
      ...
      if (this.props.editing.meta.src && this.props.editing.meta.src.id === id) {
        return resolve(true);
      }
  ...
  render () {
    ...
    const { testCases, project, projects, player, status, blocks, suites } = this.props;
    ...
    const menu = (
      <Menu theme='dark' className='folder-sidebar' selectable={false}>
        <Menu.Item className='add-folder' onClick={this.onClickNewProject.bind(this)}>
          Add New Project<Icon type="plus" />
          </Menu.Item>
        {projects.map(p => (
          <Menu.Item
            key={p.id}
          ...
          >
            <span onClick={this.props.selectProject.bind(null, p)} style={{width: '100%'}}>
              {p.name}
            </span>
            <span>
            <Tooltip title="Edit Project">
                <Icon type="edit" style={{'marginRight': '15px'}} onClick={this.onClickEditProject.bind(this, p.id)}/>
              </Tooltip>
              <Tooltip title="Delete Project">
                <Icon type="delete" onClick={this.onClickDeleteProject.bind(this, p)} />
              </Tooltip>
            </span>
          </Menu.Item>
        ))}
    ...
    return (
      <div className="side-bar">
        ...
        <Menu theme="dark"
          ...
          >
          <SubMenu
            title={<span><Icon type="file" /><span>Test Cases</span></span>}
            key={C.EDITOR_STATUS.TESTS}
            className="test-sidebar"
            >
            <Menu.Item
              onClick={this.changeTestCase.bind(this, undefined)}
              >
              <Icon type='plus'/><span>New Test</span>
            </Menu.Item>
            {testCases.map(tc => (
            <Menu.Item
          ...
          <SubMenu
            title={<span><Icon type="switcher" /><span>Blocks</span></span>}
            className="block-sidebar"
            key={C.EDITOR_STATUS.BLOCKS}
            >
            <Menu.Item
              onClick={this.changeBlock.bind(this, undefined)}
              >
              <Icon type='plus'/><span>New Block</span>
            </Menu.Item>
          ...
          <SubMenu
              title={<span><Icon type="database" /><span>Suites</span></span>}
              className="block-sidebar"
              key={C.EDITOR_STATUS.SUITES}
              >
              <Menu.Item
                onClick={this.changeSuite.bind(this, undefined)}
                >
                <Icon type='plus'/><span>New Suite</span>
              </Menu.Item>
              {suites.map(suite => (
                <Menu.Item
                  key={suite.id}
