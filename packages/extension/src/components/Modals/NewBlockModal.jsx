...
import {allRepos, getGithubRepoFromBlockShareConfig} from '../../actions/utilities'
...
export default class NewBlockModal extends React.Component {
    ...
    readGithubReposIfNecessary = () => {
        if(!this.state.repoContentsReadPending || this.props.blockShareConfigError) {
            return;
        }
    ...
    importBlock = () => {
        ...
        this.props.importBlock(this.state.allLinks[link])
    ...
    _updateSearch = (text) => {
      ...
      this.setState({foundLinks: Object.keys(this.state.allLinks).map(function(link){
        if(text.length > 0 && link.toLowerCase().indexOf(text.toLowerCase()) >= 0) 
      ...
      return (
        <Modal
          ...
          <div>
            ...
            <Button onClick={this.props.editNewBlock} block>Create a new block</Button>
            <br /><br />
            <div style={{textAlign: 'center'}}>- OR -</div>
            <br />
            Import a block from the registry
            <Input.Search data-testid={"newBlockModal_importInputSearch"} disabled = {this.state && (this.state.importing || this.props.blockShareConfigError)} placeholder={"Link"} value = {this.state.searchText} size = "large" enterButton = "Import" onSearch={this.importBlock} onChange={this.updateSearch}/>
            <Menu onClick={this.selectedBlock}>
              {this.state.foundLinks}
            </Menu>
            { this.props.blockShareConfigError ? blockShareConfigError() : <span></span>}
          </div>
        </Modal>
      )
    }
