...
class RenameModal extends React.Component {
  ...
  onRename () {
    ...
    switch (this.props.editorStatus) {
      ...
      case C.EDITOR_STATUS.BLOCKS:
        this.props.renameBlock(this.state.rename)
          .then(successFn)
          .catch(e => message.error(e.message, 1.5))
        break
      case C.EDITOR_STATUS.SUITES:
        this.props.renameSuite(this.state.rename)
          .then(successFn)
          .catch(e => message.error(e.message, 1.5))
        break
    }
  ...
  render () {
    const type = this.props.editorStatus === C.EDITOR_STATUS.SUITES
      ? 'suite' : this.props.editorStatus === C.EDITOR_STATUS.BLOCKS
        ? 'block' : 'test case'
