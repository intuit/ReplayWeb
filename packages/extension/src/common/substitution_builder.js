    ...
    toBeReplaced.map(function(singleRowOfSubs, index) {
      try {
        ...
        if (e.message.indexOf('does not exist') === -1)
          throw new Error(
            'Error occured when building context for substitutions. Error: ',
            e
          )
      }
      if (!singleRowOfSubs) return null
      return Promise.all(
        singleRowOfSubs.map(function(singleSub) {
          return doReplace(singleSub[0], context)
        })
      )
    })
  ).then(function(replacements) {
    const formattedReplacements = replacements.map(
      (replacement, valueIndex) => {
        if (!replacement) {
          return null
        }
