import {getValuesFromCommands,
        pullValuesToBeReplaced,
        removeDupeReplacements,
        isEquivState,
        updateSubstitutions,
        buildContext} from './../../src/common/substitution_builder'
...
describe("substitution builder", () => {
    describe("getValuesFromCommands", () => {
        ...
        it('should pull out values from type commands', () => {
            const expected = [null, null, "best {topic}verere {random}", null, "{fName} {lName} enjoys {random} on {todaysDate}", "{ssn} {random} {ssn} {ssn}", null, "{ssn}", null, "{game} {ssn} sadfadfas {random} {millis}"]
    ...
    describe("removeDupeReplacements", () => {
        ...
        it('should remove duplicate keys', () =>{
            ...
            expect(cleaned.length == 2 && cleaned[0].length == 2 && cleaned[0][0][0] == "a" && cleaned[0][1][0] == "b").toEqual(true)
        })
    })
