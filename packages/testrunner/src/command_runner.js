...
export async function runCommand(
  ...
  await hooks.beforeCommand.promise(
    { command, parameters: finalParameters },
    context,
    browser
  )
  const tools = {
  ...
  switch (command) {
    ...
    case 'assertCheckboxState': {
      ...
      return getAndWaitForElement(
        ...
        return Promise.all([
          ...
          return expect(
            type === 'checkbox' && Boolean(checked) === Boolean(expected)
          ).to.equal(true)
        })
      })
    }
    ...
    case 'assertElementNotVisible': {
      ...
      return browser
        .$(getSelector(target))
        .then(el =>
          el.waitForDisplayed(parseInt(timeout) || implicitWait, true)
        )
    }
    ...
    case 'assertStyle': {
      ...
      const newTarget =
        target.indexOf('id=') === 0
          ? target.replace('id=', 'identifier=')
          : target
      const execString = `return window.getComputedStyle(document.querySelector(\`${getSelector(
        newTarget
      )}\`))['${property}']`
      return getAndWaitForElement(
        target,
        ...
        .then(computed => {
          if (computed === undefined) {
            return Promise.reject(
              new Error(`style '${property}' not found on element ${target}`)
            )
          }
    ...
    case 'assertTableNotEmpty': {
      ...
      return fetchTable(target).then(table =>
        expect(table).to.have.length.of.at.least(1)
      )
    }
    ...
    case 'assertText': {
      ...
      return getAndWaitForElement(
        ...
        Promise.all([el.getText(), el.getValue()]).then(([text, value]) => {
          const data = text !== '' ? text : value || ''
          const re = expected.match(/^\/.+\/$/)
            ? regExpMatch(expected, data)
            : expected
    ...
    case 'assertTitle': {
      ...
      return browser.getTitle().then(title => expect(title).to.equal(expected))
    }

    case 'assertUrl': {
      const { expected } = finalParameters
      return browser.getUrl().then(url => expect(url).to.equal(expected))
    }
    ...
    case 'mouseEvent': {
      ...
      return ['mouseover', 'mousedown', 'mouseup', 'click']
        ...
        .catch(e => {
          if (
            !e.message.includes("Cannot read property 'dispatchEvent' of null")
          ) {
            throw e
          }
        })
    ...
    case 'setSession': {
      const { key, value } = finalParameters
      return browser
        .execute(setSessionStorage, key, value)
    ...
    default: {
      throw new Error(
        `Command "${command}" not supported yet. Do you need to update  @replayweb/testrunner?`
      )
    }
  }
}
