...
describe('runInParallelAllRegions', () => {
  afterEach(() => {
    ...
    runInParallelAllRegionsRewire.__ResetDependency__(
      'expandLoginBlockForAllRegions'
    )
  })
  ...
  describe('runInParallelAllRegions', () => {
    it('should throw error if no json tests found', () => {
      ...
      expect(
        runInParallelAllRegions.bind(null, './tests', {
          loginBlockPath: './loginBlocks'
        })
    ...
    it('should throw error if loginBlocks path is undefined', () => {
      ...
      runInParallelAllRegionsRewire.__Rewire__(
        'getJsonTestFiles',
        testsPath => ['Test1.json']
      )
      runInParallelAllRegionsRewire.__Rewire__(
        'getAllBlockContents',
        path => []
      )
      expect(runInParallelAllRegions.bind(null, './tests')).toThrow(
    ...
    it('should throw error if loginBlocks does not exist', () => {
      ...
      expect(
        ...
        'loginBlockPath is defined in replay.config.json but the path does not exist'
      )
    })
    it('should throw error if no login blocks', () => {
      ...
      ).toThrow('No login blocks found in path')
    })
    it('should write out test files with no blocks', () => {
      ...
      runInParallelAllRegionsRewire.__Rewire__(
        'expandLoginBlockForAllRegions',
        (commands, blocks) => [
          { locale: 'en-ca', commands: [{ key: 'value' }] }
        ]
      )
      runInParallelAllRegionsRewire.__Rewire__('getAllBlockContents', path => {
      ...
      const testFiles = runInParallelAllRegions('./tests', {
        loginBlockPath: './loginBlocks'
      })
    ...
    it('should write out test files with login blocks and blocks', () => {
      ...
      runInParallelAllRegionsRewire.__Rewire__(
        'getAllBlockContents',
        testsPath => {
          if (testsPath.includes('blocks'))
            return ['{"Commands": [{"cmd": "testCommand"}]}']
          else
            return [
              '{"Commands": [{"login-cmd-1": "testCommand"}]}',
              '{"Commands": [{"login-cm2": "testCommand"}]}'
            ]
        }
      )
      runInParallelAllRegionsRewire.__Rewire__(
        ...
        (commands, blocks) => {
          return [
            {
              locale: 'en-ca',
              commands: [{ 'login-enca': 'testCommand' }, { cmd: 'value' }]
            },
            {
              locale: 'fr-ca',
              commands: [{ 'login-frca': 'testCommand' }, { cmd: 'value' }]
            }
          ]
        }
      )
    ...
    it('Should write test files with passing metadata', () => {
      ...
      runInParallelAllRegionsRewire.__Rewire__('fs', {
        ...
        writeFileSync: (path, cont) => (contents = cont),
        readFileSync: path =>
          '{"commands": [{"key": "value"}], "metadata": {"md-key": "md-value"}}'
      })
      runInParallelAllRegionsRewire.__Rewire__(
        'getJsonTestFiles',
        testsPath => ['Test1.json']
