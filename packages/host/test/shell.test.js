...
describe('Shell', () => {
  ...
  describe('promiseExec', () => {
    it('should reject if there is an error', () => {
      ...
      return expect(promiseExec('whoami')).to.eventually.be.rejectedWith(
  ...
  describe('whoami', () => {
    it('should reject if there is an error', () => {
      shellRewire.__Rewire__('promiseExec', s =>
        Promise.reject(new Error('promiseExec failed'))
      )
      return expect(whoami()).to.eventually.be.rejectedWith(
        'promiseExec failed'
      )
    })
    it('should resolve if there is no error', () => {
      shellRewire.__Rewire__('promiseExec', (s, cb) =>
        Promise.resolve('testUser1')
      )
  ...
  describe('checkExecutable', () => {
    it('should reject if there is an error', () => {
      ...
      return expect(checkExecutable('docker')).to.eventually.be.rejectedWith(
        'promiseExec failed'
      )
    })
    it('should resolve if there is no error', () => {
      ...
      return expect(checkExecutable('docker')).to.eventually.become(
  ...
  describe('buildPackage', () => {
    it('should reject if there is an error', () => {
      ...
      return expect(buildPackage()).to.eventually.be.rejectedWith(
        'promiseExec failed'
      )
    })
    it('should resolve if there is no error', () => {
